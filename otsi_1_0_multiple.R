...
otsi_multiple_self_test <- function() {
  ...
  test_data <- data.frame(
    parcel_id = c("A", "B", "C"),
    land_use = c("Agroforestry", "Forest", "Cropland"),
  ...
  stopifnot(
    ...
    identical(result$scores$assessment_type[[1]], "Partial OTSI"),
    isTRUE(
      all.equal(
    ...
    is.na(result$scores$final_otsi[[3]]),
    nrow(result$group_summary) == 3L
  )

  repeated_data <- data.frame(
    parcel_id = c("A", "A", "B", "B"),
    land_use = c("Forest", "Forest", "Cropland", "Cropland"),
  ...
  stopifnot(
    ...
    aggregated$scores$soc[aggregated$scores$parcel_id == "A"] == 2,
    aggregated$scores$root_depth[
      aggregated$scores$parcel_id == "A"
    ] == 70,
    length(aggregated$qc$repeated_units_in_input) == 2L
  )
  ...
  stopifnot(isTRUE(duplicate_error))
